...
class distLinear_pcr(nn.Module):
    def __init__(self, indim, outdim):
        super(distLinear_pcr, self).__init__()
        self.L = nn.Linear(indim, outdim, bias = False)
        self.scale = 0.09
...
class ResNet(nn.Module):
    ...
    def replace_linear(self):
        del self.linear
        self.linear = distLinear(self.nf * 8 * self.block.expansion, self.num_classes)
        self.classifier = self.linear

    def replace_pcr_linear(self):
        del self.linear
        self.linear = distLinear_pcr(self.nf * 8 * self.block.expansion, self.num_classes)
        self.classifier = self.linear

...
class LODEER(ContinualModel):
    NAME = 'lodeer'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(LODEER, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            if self.task > 0:
                ...
                loss = self.args.rho/self.task*new_inter_cls.mean() + new_intra_cls.mean() + F.cross_entropy(buf_outputs, buf_labels)
            else:
                buf_inputs, buf_labels, _, _ = self.buffer.get_data(
                    self.args.minibatch_size, transform=self.transform)
                buf_outputs = self.net(buf_inputs)
                loss = self.loss(outputs, labels) + self.loss(buf_outputs, buf_labels)
            loss /= 2.
        else:
